...
if os.getenv("NV_BASE_URL"):
    os.environ.pop("OPENAI_API_KEY", None)

WORKDIR = Path.cwd()
client = OpenAI(base_url=os.getenv("NV_BASE_URL"), api_key=os.getenv("NV_API_KEY", "dummy"))
...
def agent_loop(messages: list):
    while True:
        ...
        assistant_content = [{"type": "text", "text": msg.content}] if msg.content else []
        ...
        for block in msg.tool_calls:
            ...
            try:
                output = handler(**json.loads(block.function.arguments)) if handler else f"Unknown tool: {block.function.name}"
            except Exception as e:
                output = f"Error: {e}"
            print(f"> {block.function.name}:")
            print(str(output)[:200])
            results.append({"type": "text", "text": str(output)})
