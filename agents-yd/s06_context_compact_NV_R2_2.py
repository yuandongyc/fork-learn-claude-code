...
def auto_compact(messages: list, pending_task: str = None) -> tuple[list, str]:
    ...
    # [方案2] 使用参数传递的 pending_task
    task_hint = f"\n\nCurrent task: {pending_task}" if pending_task else ""
    compressed_messages = [
        {"role": "user", "content": 
            f"[Conversation compressed. Transcript: {transcript_path}]{task_hint}\n\n"
            f"{summary}"},
    ]
...
def agent_loop(messages: list):
    while True:
        ...
        # 获取当前用户消息作为 pending_task
        pending_task = ""
        for msg in reversed(messages):
            if msg.get("role") == "user":
                pending_task = msg.get("content", "")
                break
        
        if estimate_tokens(messages) > THRESHOLD:
            print("[auto_compact triggered]")
            messages[:], summary = auto_compact(messages, pending_task)
            return summary
